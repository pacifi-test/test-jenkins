    ...
    var methods = {
        ...
        init : function( options ) {
                    ...
                    var opt = {                        
                        ...
                        form_attr       : 'class="form-search"', //Attributos para el form
                    ...
                    return this.each(function(k) {
                        ...
                        var visible = (opt.form_data[1]) ? '' : 'hidden';
                        
                        if(opt.form_show) {
                            if(opt.form_to==undefined || opt.form_to=='') {
                                alert(gettext('No se ha definido una url para la búsqueda para el datagrid'));
                                exit();
                            }
                        ...
                        if( (opt.col_hidden && (thead.find('.'+opt.col_hidden).length > 0) ) || opt.form_show) {
                            ...
                            if(opt.form_show) {                                  
                                container.find('.btn-actions').append('<button class="btn btn-info text-bold btn-form-search"><i class="fa fa-search"></i> <span class="hidden-xs">'+gettext('Buscar')+'</span></button>');
                            ...
                            bdyRows.each(function(){
                                ...
                                if(opt.order_to != '') {
                                    if(th.attr('data-order')!=undefined) {
                                        ...
                                        asc     = opt.order_to+'&o='+order+'';
                                        desc    = opt.order_to+'&o=-'+order+'';
                                        cell.append('<div class="btn-group visible-xs"><a class="" data-toggle="dropdown" href="#"><span class="caret"></span></a><ul class="dropdown-menu pull-right"><li><a href="'+asc+'" '+opt.order_attr+' data-div="'+opt.order_container+'"><i class="fa fa-caret-up fa-pd-expand"></i>'+gettext('Ascending')+'</a></li><li><a href="'+desc+'" '+opt.order_attr+' data-div="'+opt.order_container+'"><i class="fa fa-caret-down fa-pd-expand"></i>'+gettext('Descending')+'</a></li></ul></div>');
                                    }
                                }
                            });
                        ...
                        //Si hay columnas responsivas max-width: 640px
                        if(th_responsive.length > 0) {
                            $('head').append('<style type="text/css">@media (max-width: 767px) { '+th_responsive.join('')+'}</style>');
                        }
